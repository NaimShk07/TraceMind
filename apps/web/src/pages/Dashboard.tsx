  ...
  const { data: recentCommitsResponse, isLoading: isCommitsLoading } = useQuery<{
    ...
    queryFn: async () => {
      const res = await fetch(
        `${API_BASE}/repositories/${activeRepo?.repositoryId}/commits?page=1&limit=5`,
      );
      ...
      <div className="space-y-6 animate-slideUp">
        <div>
          ...
          <p className="text-xs text-[#626875]">
            Connect a repository to index history and activate telemetry controls.
          </p>
        </div>
  ...
  return (
    <div className="space-y-6 animate-slideUp">
      ...
      <div>
        ...
        <p className="text-xs text-[#626875]">
          Real-time codebase status, telemetry metrics, and AI diagnostics.
        </p>
      </div>
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-gray-500">
                    ...
                    <span className="font-mono select-all text-[11px] truncate max-w-md">
                      {activeRepo?.path}
                    </span>
                  </div>
                </div>
                ...
                <div className="relative pl-4 space-y-4 border-l border-[#1e2030]/60">
                  {recentCommits.map((commit) => (
                    <div key={commit.hash} className="relative group text-left">
                      {/* Timeline dot */}
                      <div
                        className={`absolute -left-[20.5px] top-1.5 w-2 h-2 rounded-full border border-[#0c0d14] transition-colors
                        ${
                          activeCommitHash === commit.hash
                            ? 'bg-purple-500 border-purple-300'
                            : 'bg-[#1e2030] group-hover:bg-purple-500 group-hover:border-purple-300'
                        }`}
                      />

                      <button
                        onClick={() => navigate(`/timeline?commit=${commit.hash}`)}
                        className={`w-full text-left space-y-1 block cursor-pointer p-2 -mx-2 rounded border transition-all
                          ${
                            activeCommitHash === commit.hash
                              ? 'bg-[#161722] border-purple-500/40 shadow-sm'
                              : 'hover:bg-[#161722]/50 border-transparent'
                          }`}
                      >
