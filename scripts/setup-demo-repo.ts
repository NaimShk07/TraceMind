...
`);
runGit('git add .');
runGit('git commit -m "feat: implement base username/password login service" --date="2026-07-10T10:00:00"');
...
`);
runGit('git add .');
runGit('git commit -m "feat: integrate JWT token generation for session security" --date="2026-07-11T12:00:00"');

// --- COMMIT 3: Google OAuth ---
console.log(`Committing Commit 3/7: OAuth Provider...`);
writeFile('oauthService.ts', `
...
`);
runGit('git add .');
runGit('git commit -m "feat: integrate Google OAuth authentication provider" --date="2026-07-12T14:00:00"');

// --- COMMIT 4: Refactor Token Helpers ---
console.log(`Committing Commit 4/7: Refactor Helpers...`);
writeFile('helpers/token.ts', `
import jwt from 'jsonwebtoken';
...
`);
runGit('git add .');
runGit('git commit -m "refactor: extract JWT signing logic to token helper module" --date="2026-07-13T16:00:00"');
...
`);
runGit('git add .');
runGit('git commit -m "chore: prepare docker configuration and environment setup" --date="2026-07-14T11:00:00"');
...
`);
runGit('git add .');
runGit('git commit -m "fix: resolve socket connection leak by releasing DB client in finally block" --date="2026-07-15T15:00:00"');
